...
app.post('/api/v1/stamp', (req, res) => {
    // Falls API-Key gesendet wird (optional für Testzwecke)
    const apiKey = req.headers['x-api-key'];
    // if (apiKey !== TERMINAL_API_KEY) ... (Hier deaktiviert für Frontend-Tests ohne Hardware)

    const { cardId } = req.body;
    const db = readDB();
    const user = db.users.find(u => u.cardId === cardId);
    if (!user) return res.status(404).json({ message: "Karte unbekannt" });

    const todayStr = new Date().toISOString().split('T')[0];
    const nowTime = new Date().toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' });

    let booking = db.bookings.find(b => b.userId === user.id && b.date === todayStr);
    let type = "Kommen";

    if (!booking) {
        // Neu anlegen
        booking = {
            id: Date.now(), userId: user.id, date: todayStr, start: nowTime, end: "", 
            remarks: "", type: "valid", history: [], isEdited: false
        };
        db.bookings.push(booking);
    } else {
        if (!booking.end || booking.end === "") {
            // Gehen
            booking.end = nowTime;
            type = "Gehen";
        } else {
            // Update Gehen (Überschreiben, falls mehrmals gestempelt)
            booking.end = nowTime; 
            type = "Gehen (Update)";
        }
    }
    writeDB(db);
    console.log(`Stempelung: ${user.displayName} (${type}) um ${nowTime}`);
    res.json({ status: "success", user: user.displayName, type: type });
});
